...
import { Mail, Clock, ExternalLink, FileText, Loader2, CheckCircle2, XCircle } from "lucide-react";
...
export default function ApplicantProfileDialog({
  ...
  const handleStatusUpdate = async (status?: string) => {
    try {
      ...
      const res = await fetch(`/api/application/accept/${applicationId}`, options);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="bg-slate-900 border-white/10 text-white w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-auto p-4 sm:p-6 rounded-2xl">
        <DialogHeader>
          ...
          {loading ? (
            ...
            <>
              ...
              {fullData?.status && fullData.status !== "PENDING" && (
                <div className={`mt-6 p-3 rounded-xl border flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-500 ${
                  ...
                  <div className="relative flex h-2 w-2">
                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                      fullData.status === "ACCEPTED" ? "bg-emerald-500" : "bg-rose-500"
                    }`}></span>
                  ...
                  <p className="text-[13px] font-medium leading-tight">
                    This application is currently <span className="font-bold uppercase tracking-wider">{fullData.status}</span>. 
                    <br />
                    <span className="text-slate-400 font-normal">
                      To change this decision, click the <span className={fullData.status === "ACCEPTED" ? "text-rose-400" : "text-emerald-400"}>{fullData.status === "ACCEPTED" ? "Reject" : "Accept"}</span> button below.
                    </span>
                  </p>
                </div>
              )}
              ...
              {/* Action Buttons */}
              <div className={`${fullData?.status && fullData.status !== "PENDING" ? "pt-4" : "pt-6"} border-t border-white/10 flex flex-col sm:flex-row justify-end animate-in fade-in slide-in-from-bottom-4 duration-500 gap-3 mt-4`}>
                <Link
                  href={fullData?.resumeUrl || basicData.resumeUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-medium transition-all border border-white/10 w-full sm:w-auto order-2 sm:order-1"
                >
