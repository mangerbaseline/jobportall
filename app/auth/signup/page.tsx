    ...
    <div className="relative min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] shrink-0 p-12 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/3 left-1/4 w-72 h-72 bg-violet-600/10 dark:bg-violet-600/20 rounded-full blur-3xl" />
          <div className="absolute bottom-1/4 right-1/4 w-56 h-56 bg-indigo-600/8 dark:bg-indigo-600/15 rounded-full blur-3xl" />
        </div>
        ...
        <div className="relative z-10 inline-flex items-center gap-3 glass-card rounded-2xl px-5 py-3">
          <div className="flex -space-x-2">
            {["A", "B", "C"].map((l) => (
              <div
                key={l}
                className="w-8 h-8 rounded-full brand-gradient border-2 border-card flex items-center justify-center text-xs font-bold text-white"
              >
                {l}
              </div>
            ))}
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-bold text-foreground">4,500+</span> members joined
            this month
          </p>
        </div>
      </div>
