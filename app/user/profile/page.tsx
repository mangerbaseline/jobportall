    ...
    <div className="max-w-5xl mx-auto mt-20 p-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-card text-card-foreground p-8 rounded-2xl shadow-sm border border-border flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div className="flex-1 text-center md:text-left space-y-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{data.name || "Unknown User"}</h1>
            <p className="text-muted-foreground font-medium uppercase tracking-wider text-sm mt-1">{data.role}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
            ...
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium ${data.employed ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400'}`}>
              <Briefcase className="w-4 h-4" />
              {data.employed ? "Employed" : "Unemployed"}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
            ...
            {(!data.personal || data.personal.length === 0) && (
              <Link href="/user/personal/new" className="text-sm flex items-center gap-1.5 text-primary hover:text-primary/80 transition-colors bg-primary/10 px-3 py-1.5 rounded-full font-medium">
          ...
          {data.personal && data.personal.length > 0 ? (
                ...
                {data.personal[0].phone && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">{data.personal[0].phone}</p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">
                        {[data.personal[0].city, data.personal[0].state, data.personal[0].country].filter(Boolean).join(', ')}
                      </p>
                    </div>
                  </div>
                )}
                {data.personal[0].website && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a href={data.personal[0].website.startsWith('http') ? data.personal[0].website : `https://${data.personal[0].website}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline truncate block max-w-[150px]">
                        {data.personal[0].website}
                      </a>
                    </div>
                  </div>
                ...
                {data.personal[0].dob && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">{new Date(data.personal[0].dob).toLocaleDateString()}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-border rounded-lg bg-muted/20">
              ...
              <p className="text-muted-foreground text-sm font-medium">No personal details provided yet.</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col h-full">
          <div className="flex items-center justify-between mb-6">
            ...
            {(!data.professional || data.professional.length === 0) && (
              <Link href="/user/professional/new" className="text-sm flex items-center gap-1.5 text-primary hover:text-primary/80 transition-colors bg-primary/10 px-3 py-1.5 rounded-full font-medium">
                <PlusCircle className="w-4 h-4" />
                Add Details
              </Link>
            )}
          ...
          {data.professional && data.professional.length > 0 ? (
            <div className="space-y-5 flex-1">
              <div className="pb-4 border-b border-border">
                <h3 className="font-bold text-lg">{data.professional[0].title || 'Professional Title'}</h3>
                {data.professional[0].company && (
                  <div className="flex items-center gap-2 text-muted-foreground mt-1">
                    <Building className="w-4 h-4" />
                    <span className="text-sm font-medium">{data.professional[0].company}</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {data.professional[0].experience !== null && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">{data.professional[0].experience} Years</p>
                    </div>
                  </div>
                )}
                {data.professional[0].skills && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium truncate max-w-[150px]" title={data.professional[0].skills}>{data.professional[0].skills}</p>
                    </div>
                  </div>
                )}
                {data.professional[0].education && (
                  <div className="flex items-start gap-3 sm:col-span-2">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Education</p>
                      <p className="text-sm font-medium line-clamp-2">{data.professional[0].education}</p>
                    </div>
                  </div>
                )}
                 {(data.professional[0].currentSalary !== null || data.professional[0].expectedSalary !== null) && (
                  <div className="flex items-start gap-3 sm:col-span-2">
                    <DollarSign className="w-4 h-4 text-muted-foreground mt-0.5 shrink-0" />
                    <div>
                      <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Salary Stats</p>
                      <p className="text-sm font-medium">
                        {data.professional[0].currentSalary && `Current: $${data.professional[0].currentSalary} `}
                        {data.professional[0].expectedSalary && `| Expected: $${data.professional[0].expectedSalary}`}
                      </p>
                    </div>
                  </div>
                )}
             ...
             <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-border rounded-lg bg-muted/20">
              ...
              <p className="text-muted-foreground text-sm font-medium">No professional details provided yet.</p>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{data._count?.applications || 0}</p>
          </div>
        </div>
      </div>
      ...
      {data.jobs && data.jobs.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.jobs.map((job) => (
              <div key={job.id} className="bg-card border border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{job.vacancy} Vacanc{job.vacancy === 1 ? 'y' : 'ies'}</span>
                  </div>
